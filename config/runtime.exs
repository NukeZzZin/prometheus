...
source!([Path.join(env_dir_prefix, ".env"), Path.join(env_dir_prefix, ".env.#{config_env()}"), System.get_env()])

config :joken, default_signer: jwt_secret_key

config :snowflake,
  epoch: 1_767_268_800, # ! 2026-01-01 12:00:00 (default)
  machine_id: env!("MACHINE_ID", :integer, :erlang.phash2(:erlang.node(), 1024))

if env!("PHX_SERVER", :boolean, false), do: config(:prometheus, PrometheusEntry.Endpoint, server: true)

postgres_pool_size = env!("POSTGRES_POOL_SIZE", :integer, :erlang.system_info(:schedulers_online) * 2)
...
case env!("DATABASE_URL", :string?, nil) do
  ...
  database_url ->
    config :prometheus, Prometheus.Repository,
      url: database_url,
      socket_options: if(env!("ECTO_IPV6", :boolean, false), do: [:inet6], else: []),
      pool_size: postgres_pool_size
end

redis_pool_size = env!("REDIS_POOL_SIZE", :integer, :erlang.system_info(:schedulers_online) * 2)

config :prometheus, Prometheus.Redis,
  host: env!("REDIS_HOST", :string, "localhost"),
  password: env!("REDIS_PASSWORD", :string, "redis"),
  port: env!("REDIS_PORT", :integer, 6379),
  pool_size: redis_pool_size

phoenix_server_port = env!("PHX_PORT", :integer, 4000)

case config_env() do
  :dev ->
    config :prometheus, Prometheus.Repository,
      database: env!("POSTGRES_DB", :string, "prometheus_dev")
  ...
  :prod ->
    config :prometheus, Prometheus.Repository,
      database: env!("POSTGRES_DB", :string, "prometheus_prod"),
      ssl: true

    config :prometheus, PrometheusEntry.Endpoint,
      http: [ip: {0, 0, 0, 0, 0, 0, 0, 0}, port: phoenix_server_port],
      # url: [host: env!("PHX_HOST", :string, "prometheus.com"), port: 443, scheme: "https"],
      secret_key_base: secret_key_base

  :test ->
    config :prometheus, Prometheus.Repository,
      database: env!("POSTGRES_DB", :string, "prometheus_test") <> env!("MIX_TEST_PARTITION", :string, "1")
